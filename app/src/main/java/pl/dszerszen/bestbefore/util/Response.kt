...
sealed class Response<T>(open val data: T?, open val errorMessage: StringValue? = null) {
    ...
    class Error<T>(override val data: T?, override val errorMessage: StringValue) : Response<T>(null, errorMessage)
}

fun <T> T.asSuccess(): Response.Success<T> = Response.Success(this)
fun <T> T.asError(message: StringValue): Response.Error<T> = Response.Error(null, message)